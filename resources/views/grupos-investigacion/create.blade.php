    ...
    <div class="container mx-auto">
        <div class="bg-muted flex min-h-svh flex-col items-center justify-center gap-6 ">
            <div class="flex w-full max-w-md flex-col gap-6 p-5 dark:bg-neutral-700 bg-gray-100  rounded-xl">
                ...
                <form action="{{ route('grupos-investigacion.store') }}" method="POST">
                    @csrf
                    @method('POST')
                    <div class="mb-4">
                        <flux:input label="Nombre" type="text" value="{{old('nombre')}}" name="nombre" id="nombre"
                            placeholder="Nombre del periodo" />
                    </div>
                    <div class="mb-4">
                        <flux:input label="Correo" type="email" value="{{old('correo')}}" name="correo" id="correo"
