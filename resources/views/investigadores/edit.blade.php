        ...
        <div class="bg-muted flex min-h-svh flex-col items-center justify-center gap-6 ">
            <div class="flex w-full max-w-md flex-col gap-6 p-5 dark:bg-neutral-700 bg-gray-100  rounded-xl">
                ...
                <div class="flex w-full flex-col text-center mb-4">
                    <flux:heading size="xl">Actualizar Investigador</flux:heading>
                    {{-- <flux:subheading>Registrar un nuevo Investigador.</flux:subheading> --}}
                </div>
                <form action="{{ route('investigadores.update', $investigador) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <flux:input label="Nombre" type="text" value="{{old('name',$investigador->name)}}" name="name" id="name"
                            placeholder="Nombre del investigador" />
                    </div>
                    <div class="mb-4">
                        <flux:input label="Correo" type="email" value="{{old('email',$investigador->email) }}" name="email" id="email"
                    ...
                    <div class="mb-4">
                        <flux:select label="Grupo de Investigacion" name="grupo_investigacion_id" value="" wire:model="industry" placeholder="Seleccione grupo...">
                            ...
                            <flux:select.option value="{{$grupo->id}}" selected="{{$investigador->grupo_investigacion_id == $grupo->id}}" >{{ $grupo->nombre }}</flux:select.option>
                            @endforeach
                        </flux:select>
                    </div>
                    <div class="mb-4">
                        <flux:select label="Tipo/Rol" name="role" value="" wire:model="industry" placeholder="Seleccione grupo...">
                            <flux:select.option value="Investigador" selected="{{$investigador->role == 'Investigador'}}" >{{ __('Investigador') }}</flux:select.option>
                            <flux:select.option value="Lider Grupo" selected="{{$investigador->role == 'Lider Grupo'}}" >{{ __('Lider Grupo') }}</flux:select.option>
                        </flux:select>
                    </div>
                    <div class="flex">
                        <flux:spacer />
